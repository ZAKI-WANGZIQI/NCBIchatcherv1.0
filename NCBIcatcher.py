...
class NCBIFastaDownloader:
    ...
    def __init__(self, download_dir=BASE_DOWNLOAD_DIR, headless=HEADLESS_MODE, enable_resume=ENABLE_RESUME):
        ...
        self.downloaded_pages = set()  # 记录已下载的页码
    ...
    def setup_driver(self, search_term):
        ...
        try:
            ...
            if self.enable_resume:
                self._load_downloaded_pages(search_term)
    ...
    def _load_downloaded_pages(self, search_term):
        """加载已下载的页码记录"""
        record_file = os.path.join(self.current_download_dir, "downloaded_pages.json")
        if os.path.exists(record_file):
            try:
                ...
                print(f"加载断点续传记录: 已下载 {len(self.downloaded_pages)} 页，当前页码: {self.current_page}")
            except Exception as e:
                print(f"加载断点续传记录失败: {e}")
    ...
    def _save_downloaded_pages(self, search_term):
        """保存已下载的页码记录"""
    ...
    def download_fasta_protein(self, search_term):
        ...
        downloaded_files = []
        page_num = self.current_page  # 从当前页码开始
        ...
        try:
            ...
            while page_num <= MAX_PAGES:
                ...
                # 检查是否已下载过该页（断点续传）
                if self.enable_resume and page_num in self.downloaded_pages:
                    print(f"第 {page_num} 页已下载，跳过...")
                    page_num += 1
                    continue
                ...
                if downloaded_file:
                    ...
                    # 记录已下载的页码
                    if self.enable_resume:
                        self.downloaded_pages.add(page_num)
                        self.current_page = page_num + 1  # 更新当前页码为下一页
                        self._save_downloaded_pages(search_term)
